...
import { TimeScroller } from "@/components/ui/time-input";
...
const ShippingConfigurations = () => {
    ...
    const handleFreeShippingMethod = (methodDbKey: string) => { 
        if (methodDbKey === config.freeShippingMethod) {
            setConfig(prev => ({
                ...prev,
                freeShippingMethod: ""
            }));
    ...
    return (
        <div className="max-w-6xl mx-auto p-4 space-y-6 border-2">
            ...
            <div className="bg-white rounded-lg shadow p-6 border-2 space-y-4">
                ...
                {config.shippingMethods.sameDayDelivery && (
                    <div className="border-2 p-4 flex flex-col md:flex-row lg:flex-row rounded-lg md:space-x-2 space-y-4 md:space-y-0 lg:space-y-0">
                        <div className="flex-1">
                            <label>Cut-off time for same day delivery: </label>
                            <TimeScroller
                                value="2:30 PM Africa/Lagos"
                                onChange={(time) => console.log(time)} // "2:30 PM Africa/Lagos"
                                timeFormat="12h"
                                withTimezone
                            />
                        </div>
                        <div className="flex-1">
                            <label>Time Zone: </label>
                                <Input
                                    type="time"
                                    value={config.ifSameDay.cutOffTime}
                                    className="border-2 block flex-1 w-full"
                                    min="09:00" 
                                    max="18:00"
                                    required
                                />
